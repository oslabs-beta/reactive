    ...
    const traverseForStats = (node) => {
        if (node.type && typeof node.type === "string") {
            ...
            if (node.type.toLowerCase().includes("function"))
                functionalCount++;
            else if (node.type.toLowerCase().includes("class"))
                classCount++;
            else
                nullCount++;
        }
...
window.addEventListener("message", (event) => {
    if (event.data.type === "astData") {
        ...
        */
        root.render(react_1.default.createElement("div", { className: "h-screen w-screen" },
            react_1.default.createElement(AppHeader_1.default, { key: appName, stats: stats, appName: appName, filePath: filePath }),
            astData && Object.keys(astData).length > 0 ? (react_1.default.createElement(Dendrogram_1.default, { data: astData })) : (react_1.default.createElement("p", null, "No component data to display"))));
    }
});
