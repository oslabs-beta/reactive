...
suite('Complete Tests', function () {
    ...
    suite('React Visualizer Extension', function () {
        ...
        test('Extension should be present and active', async function () {
            const extension = vscode.extensions.all.find(ext => ext.id.toLowerCase().includes('reactive'));
        ...
        test('Commands should be registered', async function () {
            ...
            const reactiveCommands = commands.filter(cmd => cmd.includes('reactive'));
            ...
            expectedCommands.forEach(cmd => assert.ok(commands.includes(cmd), `Command ${cmd} should be registered`));
        });
    });
...
// // src/test/suite/completeTests.test.ts
